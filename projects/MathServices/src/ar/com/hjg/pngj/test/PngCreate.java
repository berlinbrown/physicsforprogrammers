...
public class PngCreate {
	...
	private static void makeTestImage(PngWriter png) throws Exception {
		...
		for (; iline.getRown() < rows; iline.incRown()) {
			for (int j = 1; j < cols - 1; j++) {
				ImageLineHelper.setValD(iline, j * channels,
						clamp((2 * j / cols) - 0.3, 0, 1.0));
				ImageLineHelper.setValD(iline, j * channels + 1,
						clamp((2 * iline.getRown() / rows) - 0.4, 0, 1.0));
				ImageLineHelper.setValD(
						iline,
						j * channels + 2,
						clamp((0.55 * Math.sin(13.0 * iline.getRown() / rows
								+ j * 25.0 / cols) + 0.5), 0, 1.0));
			}
			if (alpha)
				addAlpha(iline);
			png.writeRow(iline);
		...
		for (int i = 0; i < iline.imgInfo.cols; i++) {
			...
			if (i == 0 || i == iline.imgInfo.cols - 1 || iline.getRown() < 2)
				...
				double d = Math.sqrt(((0.5 * i) / cols + 0.0)
						+ ((0.5 * (rows - iline.getRown())) / rows + 0.0)); 
				d = d * 1.3 - 0.2;
				alpha = clamp(d, 0.0, 1.0);
			}
			ImageLineHelper.setValD(iline, i * 4 + 3, clamp(alpha, 0, 1.0)); 
		}
	}
	...
	public static void createTest1(String orig, int cols, int rows, int bitspc,
			int channels) throws Exception {
		if (channels != 3 && channels != 4)
			throw new RuntimeException("bad channels number (must be 3 or 4)");
		PngWriter i2 = FileHelper.createPngWriter(new File(orig),
				new ImageInfo(cols, rows, bitspc, channels == 4), true);
		makeTestImage(i2);
	...
	public static void createBlackRGB8(String filename, int cols, int rows)
			throws Exception {
		PngWriter png = FileHelper.createPngWriter(new File(filename),
				new ImageInfo(cols, rows, 8, false), true);
		png.setCompLevel(3);
		ImageLine iline = new ImageLine(png.imgInfo);
